...
public void registerPatient(String name, String gender, String address, String dob, String phone) {
    try {
        //  Normalize the patient name: remove spaces & make uppercase
        ...
        if (rs.next()) {
            ...
            if (existingPhone.equals(phone)) {
                JOptionPane.showMessageDialog(null, 
                    "⚠ Patient already registered with this phone number!\nName: " + rs.getString("P_NAME"));
                return;
            }
        }
        ...
        pst.setDate(4, java.sql.Date.valueOf(dob)); // ✅ YYYY-MM-DD
        pst.setString(5, phone);
        pst.setBoolean(6, false); // ADMITTED default false
        pst.executeUpdate();
    ...
    public void bookAppointment(String patientName, String doctorName, String dateStr, String timeStr) {
        try {
            ...
            pst.setDate(5, java.sql.Date.valueOf(dateStr)); // ✅ YYYY-MM-DD
            pst.setTime(6, java.sql.Time.valueOf(timeStr)); // ✅ HH:MM:SS
            pst.setString(7, "BOOKED");
            pst.executeUpdate();
            JOptionPane.showMessageDialog(null, "✅ Appointment Booked!");
        } catch (IllegalArgumentException ie) {
            JOptionPane.showMessageDialog(null, "❌ Date must be YYYY-MM-DD and time HH:MM:SS");
        } catch (Exception e) {
    ...
    public DefaultTableModel getAppointmentsTableModel() {
        ...
        try {
            ...
            while (rs.next()) {
                ...
                java.sql.Date d = rs.getDate("APP_DATE"); // ✅ sql.Date
                java.sql.Time t = rs.getTime("APP_TIME"); // ✅ sql.Time
    ...
    public boolean updateAppointment(int appointmentId, String newDateStr, String newTimeStr, String newStatus) {
        try {
            ...
            PreparedStatement pst = conn.prepareStatement(q);
            pst.setDate(1, java.sql.Date.valueOf(newDateStr)); // ✅ sql.Date
            pst.setTime(2, java.sql.Time.valueOf(newTimeStr)); // ✅ sql.Time
    ...
    public DefaultTableModel getDoctorScheduleModel(String doctorName) {
        ...
        try {
            ...
            JOptionPane.showMessageDialog(null, "❌ Error fetching doctor schedule: " + e.getMessage());
        }
        return new DefaultTableModel(data, columns) {
            @Override public boolean isCellEditable(int r, int c) { return false; }
        };
    ...
    public int generateToken(String patientName, String doctorName) {
        try {
            ...
            JOptionPane.showMessageDialog(null, "🎫 Token generated: " + nextToken + " for " + patientName);
            return nextToken;
        } catch (Exception e) {
            JOptionPane.showMessageDialog(null, "❌ Error generating token: " + e.getMessage());
            return -1;
        }
    }
    ...
    public boolean transferToDoctorQueue() {
        try {
            ...
            if (rs.next()) {
                ...
                if (u > 0) {
                    JOptionPane.showMessageDialog(null, "➡️ Patient moved to IN_PROGRESS (QUEUE_ID=" + qid + ")");
                    return true;
                }
            } else {
                JOptionPane.showMessageDialog(null, "⚠ No patients in waiting queue.");
            }
        } catch (Exception e) {
            JOptionPane.showMessageDialog(null, "❌ Error transferring to doctor queue: " + e.getMessage());
        }
        return false;
    }
